...
<!--      </let-table-column>-->
    ...
    <let-table v-if="serverNotifyList && showOthers"
               :data="serverNotifyList" :title="$t('serverList.title.serverStatus')" :empty-msg="$t('common.nodata')"
    ...
    <let-modal
      ...
      <let-button size="small" theme="primary" class="tbm16" @click="configServant()">{{$t('operate.add')}} Servant
      </let-button>
      <let-table v-if="servantModal.model" :data="servantModal.model" :empty-msg="$t('common.nodata')">
        ...
        <let-table-column :title="$t('operate.operates')" width="90px">
          <template slot-scope="scope">
            ...
            <let-table-operation class="danger" @click="deleteServant(scope.row.id)">{{$t('operate.delete')}}
            </let-table-operation>
          </template>
        </let-table-column>
      </let-table>
    ...
    <let-modal
      ...
      <let-form v-if="moreCmdModal.model" ref="moreCmdForm">
        <let-form-item itemWidth="100%">
          <let-radio v-model="moreCmdModal.model.selected" label="setloglevel">{{$t('serverList.servant.logLevel')}}
          </let-radio>
          <let-select
            size="small"
        ...
        <let-form-item itemWidth="100%">
          <let-radio v-model="moreCmdModal.model.selected" label="loadconfig">{{$t('serverList.servant.pushFile')}}
          </let-radio>
          <let-select
            size="small"
            ...
            <let-option v-for="l in moreCmdModal.model.configs" :key="l.filename" :value="l.filename">{{l.filename}}
            </let-option>
          </let-select>
        </let-form-item>
        <let-form-item itemWidth="100%">
          <let-radio v-model="moreCmdModal.model.selected" label="command">{{$t('serverList.servant.sendCommand')}}
          </let-radio>
          <let-input
            size="small"
