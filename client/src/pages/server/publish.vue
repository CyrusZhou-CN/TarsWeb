<template>
  <div class="page_server_publish">
    ...
    <div>
      ...
      >
        ...
        <div slot="operations" style="margin-left: -15px;">
          ...
          >
          &nbsp;&nbsp;
          <let-button size="small" @click="gotoPackage">{{
            $t("managePackage.title")
          }}</let-button>
        </div>
      </let-table>
      <!-- 发布服务弹出框 -->
        ...
        <let-form v-if="publishModal.model" ref="publishForm" itemWidth="100%">
          ...
          >
            <div>
              ...
              >
                <let-option
                  v-for="(d, index) in publishModal.model.patchList"
                  :key="d.id"
                  :value="d.id"
                  :title="
                    d.id +
                ...
                >
                  <div>
                    <span v-html="imgNew" v-if="index == 0"></span
                    ><span v-else v-html="imgSpace">&emsp;</span>
                    <span
                      v-html="imgCur"
                      v-if="includes(nowVersion, d.id)"
                    ></span
                    ><span v-else v-html="imgSpace"></span> {{ d.id }}|
                    {{ "PubTime:" + d.publish_time }} |
          ...
          <let-form-item :label="$t('serverList.table.th.version')" v-else>
            ...
            <let-button
              ...
              >{{ $t("pub.dlg.compileAndPublish") }}</let-button
            >
            <let-button
              size="small"
              class="mt10"
    ...
    >
      ...
      >
        <let-table-column
          :title="$t('managePackage.table.th.c1')"
          prop="id"
        ></let-table-column>
        <let-table-column
          :title="$t('managePackage.table.th.c2')"
          prop="server"
        ></let-table-column>
        <let-table-column
          :title="$t('managePackage.table.th.c7')"
          prop="upload_user"
        ></let-table-column>
        <let-table-column
          :title="$t('managePackage.table.th.c3')"
          prop="upload_time"
        ></let-table-column>
        <let-table-column
          :title="$t('managePackage.table.th.c4')"
          prop="comment"
        ></let-table-column>
        <let-table-column
          :title="$t('managePackage.table.th.c5')"
          prop="publish_time"
        ></let-table-column>
        <let-table-column :title="$t('managePackage.table.th.c6')">
          <template slot-scope="scope">
            <let-table-operation>
              <a
                class="let-table__operation"
                target="_blank"
                :href="
                  `/pages/server/api/download_package?id=${scope.row.id}&name=${scope.row.tgz}`
                "
                >{{ $t("operate.download") }}</a
              >
            </let-table-operation>
        ...
        <let-pagination
          slot="pagination"
          align="right"
          :total="packageTotalPage"
          :page="packagePage"
          @change="changePackagePage"
        >
        </let-pagination>
        <div slot="operations" style="margin-left: -15px;">
          <let-button
    ...
    >
      <let-form @submit.native.prevent="getHistoryList">
        <let-form-item itemWidth="100%" :label="$t('pub.date')">
          <let-date-range-picker
            ...
            :end.sync="endTime"
          ></let-date-range-picker>
          <let-button type="submit" theme="primary" style="margin-left:20px">{{
            $t("operate.search")
          }}</let-button>
        </let-form-item>
      </let-form>
      ...
      >
        <let-table-column
          :title="$t('historyList.table.th.c1')"
          prop="create_time"
        ></let-table-column>
        <let-table-column
        ...
        <let-table-column
          :title="$t('historyList.table.th.c3')"
          prop="userName"
        ></let-table-column>
        ...
        <let-table-column :title="$t('historyList.table.th.c4')">
          <template slot-scope="scope">
            ...
            <let-table-operation @click="deleteTask(scope.row.task_no)">{{
              $t("operate.delete")
            }}</let-table-operation>
          </template>
        </let-table-column>
...
export default {
  ...
  methods: {
    ...
    openPublishModal(upload) {
      ...
      this.getPatchList(first.application, first.server_name, 1, 50).then(
        (data) => {
    ...
    getNewVersionList(node) {
      return this.$ajax
        .getJSON("/server/api/server_now_version", {
    ...
    uploadFile(file) {
      const { treeid } = this;
      const app =
        treeid && treeid.split(".")[0] && treeid.split(".")[0].slice(1);
