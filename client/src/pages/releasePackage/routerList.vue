<template>
  <section class="container">
    <div>
      <let-button theme="primary" size="small" @click="uploadModal.show = !uploadModal.show">
    ...
    <let-table :data="packages.rows" :title="$t('serverList.title.serverList')" :empty-msg="$t('common.nodata')">
      ...
      <let-table-column :title="$t('operate.operates')" width="260px">
        <template slot-scope="scope">
          <let-table-operation @click="setDefault(scope.row)">{{$t('operate.default')}}</let-table-operation>
