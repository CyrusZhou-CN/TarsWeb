<template>
  <section class="page_operation_region">
    ...
    <let-table ref="table" :data="items" :empty-msg="$t('common.nodata')" :title="$t('region.list')">
      ...
      <let-table-column :title="$t('region.label')" prop="label"></let-table-column>
      <let-table-column :title="$t('operate.operates')" width="180px">
        <template slot-scope="scope">
          <let-table-operation @click="editRegion(scope.row)">{{$t('operate.update')}}</let-table-operation>
          <let-table-operation @click="removeItem(scope.row)">{{$t('operate.delete')}}</let-table-operation>
        </template>
      </let-table-column>
    </let-table>
    <let-modal :title="$t(title)" v-model="showModel" width="600px" @on-confirm="saveItem">
...
<script>
  export default {
    ...
    methods: {
      fetchData() {
        this.$ajax.getJSON('/server/api/get_region_list').then((items) => {
          this.items = items || []
        }).catch((err) => {
      ...
      saveItem() {
        if (this.$refs.detailForm.validate()) {
          ...
          const url = model.id ? '/server/api/update_region' : '/server/api/add_region';
      ...
      removeItem ({id}) {
        this.$ajax.getJSON('/server/api/delete_region', {id}).then((items) => {
          this.$tip.success(this.$t('common.success'));
          this.fetchData()
        }).catch((err) => {
          this.$tip.error(`${this.$t('common.error')}: ${err.message || err.err_msg}`);
