...
export default {
  ...
  methods: {
    fetchData() {
      ...
      return this.$ajax.getJSON('/k8s/api/query_business_relation', this.query).then((data) => {
        loading.hide();
        this.items = data;
      }).catch((err) => {
        loading.hide();
    ...
    saveItem() {
      if (this.$refs.detailForm.validate()) {
        ...
        const url = model.f_id ? '/k8s/api/update_business_relation' : '/k8s/api/add_business_relation';
    ...
    removeItem(d) {
      this.$confirm(this.$t('businessRelation.delete.confirmTips'), this.$t('common.alert')).then(() => {
        const loading = this.$Loading.show();
        this.$ajax.getJSON('/k8s/api/delete_business_relation', { f_id: d.f_id }).then(() => {
          loading.hide();
          this.fetchData().then(() => {
            this.$tip.success(this.$t('common.success'));
          });
    ...
    getBusinessData() {
      return this.$ajax.getJSON('/k8s/api/query_business').then((data) => {
        this.business = data;
      })
    },

    getApplicationData() {
      return this.$ajax.getJSON('/k8s/api/query_application').then((data) => {
